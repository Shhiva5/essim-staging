...
#define calc_window_ssim_8_int_8u_avx2() \
  { \
    /* STEP 2. adjust values */ \
    ...
    sigma_both_a = _mm256_sub_epi32(sigma_both, both_sum_mul); \
    /* STEP 3. process numbers, do scale */ \
    a = _mm256_add_epi32(both_sum_mul, halfC1); \
    b = _mm256_add_epi32(sigma_both_a, halfC2); \
    c = _mm256_add_epi32(_mm256_add_epi32(ref_sum_sqd, cmp_sum_sqd), fullC1); \
...
#define calc_window_ssim_16_int_8u_avx2() \
  { \
    /* STEP 2. adjust values */ \
    _r0 = _mm256_and_si256(sum, fullLSB); \
    _r1 = _mm256_srli_epi32(sum, 16); \
...
void sum_windows_8x4_int_8u_avx2(SUM_WINDOWS_FORMAL_ARGS) {
  ...
  size_t i = 0;
  const __m256i halfC1 = _mm256_set1_epi32(C1>>1);
  const __m256i halfC2 = _mm256_set1_epi32(C2>>1);
  const __m256i fullC1 = _mm256_set1_epi32(C1);
  ...
  for (; i + WIN_CHUNK <= numWindows; i += WIN_CHUNK) {
    ...
    calc_window_ssim_8_int_8u_avx2();
...
void sum_windows_16x4_int_8u_avx2(SUM_WINDOWS_FORMAL_ARGS) {
  ...
  for (; i + WIN_CHUNK <= numWindows; i += WIN_CHUNK) {
    ...
    calc_window_ssim_16_int_8u_avx2()
