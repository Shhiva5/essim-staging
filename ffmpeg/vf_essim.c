...
#if UPDATED_INTEGER_IMPLEMENTATION
            res = ssim_compute_ctx(essim_ctx, ref, refStride, cmp, cmpStride, roiY, roiHeight, essim_mink_value);
#else
...
#if UPDATED_INTEGER_IMPLEMENTATION
            ssim_aggregate_score(pSsimScore, pEssimScore, s->essim_ctx_array[i], essim_mink_value);
#else
